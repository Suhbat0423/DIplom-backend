...
export const options = {
  stages: [
    ...
    { duration: __ENV.STAGE_2_DURATION || "30s", target: Number(__ENV.STAGE_2_TARGET || 25) },
    { duration: __ENV.STAGE_3_DURATION || "30s", target: Number(__ENV.STAGE_3_TARGET || 50) },
    { duration: __ENV.STAGE_4_DURATION || "30s", target: Number(__ENV.STAGE_4_TARGET || 75) },
...
function createOrder(token, iterationId) {
  const response = http.post(
    ...
    JSON.stringify({
      items: [
        {
          productId: `stress-payment-product-${iterationId}`,
          storeId: __ENV.STORE_ID || "load-store-1",
          name: "Stress Payment Product",
          imageUrl: "",
      ...
      shippingAddress: {
        ...
        city: "Ulaanbaatar",
        district: "Sukhbaatar",
        addressLine1: "Stress Test Street",
        addressLine2: "",
        postalCode: "14200",
      },
      deliveryFee: Number(__ENV.DELIVERY_FEE || 5000),
      tax: Number(__ENV.TAX || 0),
      notes: `stress-payment-order-${iterationId}`,
    }),
    jsonHeaders(token),
  );
...
export default function (data) {
  ...
  const paymentCreateResponse = http.post(
    ...
    JSON.stringify({
      ...
      notes: `stress-payment-create-${iterationId}`,
      metadata: { source: "k6-stress" },
    }),
    jsonHeaders(token),
  );

  const createOk = check(paymentCreateResponse, {
  ...
  const confirmResponse = http.post(
    ...
    JSON.stringify({
      transactionId: `stress-txn-${Date.now()}-${iterationId}`,
      providerReference: `stress-provider-${iterationId}`,
      metadata: { source: "k6-stress-confirm" },
    }),
    jsonHeaders(token),
  );
